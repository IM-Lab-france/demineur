(() => {
    ...
    function renderConversations() {
        ...
        if(!visible.length){const empty=document.createElement('p');empty.className='small text-muted';empty.textContent=typeof currentGameId!=='undefined'&&currentGameId?'Chat de la partie indisponible.':'Aucune conversation.';root.appendChild(empty);}
        $('chatBadge').textContent=unread;$('chatBadge').classList.toggle('hidden',!unread);
    }
    function renderMessages() {
        const root=$('chatMessages');root.textContent='';
        messages.forEach(m=>{const item=document.createElement('article');item.className='chat-message '+(Number(m.sender_id)===ownId()?'mine':'theirs');item.dataset.messageId=m.id;const meta=document.createElement('small');meta.textContent=`${m.username||'Système'} · ${new Date(String(m.created_at).replace(' ','T')).toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'})}`;const body=document.createElement('div');body.className='chat-message-body';body.appendChild(m.deleted_at?document.createTextNode('Message supprimé'):escapeLink(m.body||''));item.append(meta,body);
    ...
    window.handleChatEvent=data=>{
        ...
        if(data.type==='chat_typing'&&Number(data.conversationId)===active){$('chatTyping').textContent=data.typing?`${data.username} écrit…`:'';}
    };
